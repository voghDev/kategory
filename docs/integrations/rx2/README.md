...
obs
//ObservableKW(observable=io.reactivex.internal.operators.observable.ObservableFromArray@75603f12)
```

```kotlin
val flow = Flowable.fromArray(1, 2, 3, 4, 5).k()
flow
//FlowableKW(flowable=io.reactivex.internal.operators.flowable.FlowableFromArray@2fe5daf0)
```

```kotlin
val subject = PublishSubject.create<Int>().k()
subject
//ObservableKW(observable=io.reactivex.subjects.PublishSubject@477753ba)
```
...
```kotlin
obs.value()
//io.reactivex.internal.operators.observable.ObservableFromArray@75603f12
```

```kotlin
flow.value()
//io.reactivex.internal.operators.flowable.FlowableFromArray@2fe5daf0
```

```kotlin
subject.value()
//io.reactivex.subjects.PublishSubject@477753ba
```
